  ...
  return (
    <group>
      ...
      <group position={[0, 0, 0]}>
        ...
        {/* Floor 1 Glowing Floor Grid */}
        <gridHelper args={[40, 20, "#38bdf8", "#1e293b"]} position={[0, 0.02, 0]} />
        ...
        <mesh position={[-7, 1.5, -2]}>
          ...
          <meshStandardMaterial color="#0284c7" transparent opacity={0.3} roughness={0.1} />
        </mesh>
      ...
      <group ref={floor2Group}>
        {/* Floor 2 Slab - Semi-transparent architectural glass so Floor 1 beds show */}
        <mesh position={[0, 7.8, 0]} receiveShadow>
          <boxGeometry args={[42, 0.3, 30]} />
          <meshStandardMaterial
            color="#090d16"
            transparent
            opacity={isAll ? 0.35 : activeFloor === 2 ? 0.85 : 0.2}
            roughness={0.2}
            metalness={0.4}
          />
        </mesh>
        <gridHelper args={[40, 20, "#10b981", "#1e293b"]} position={[0, 8.02, 0]} />
        ...
        <mesh position={[6, 9.5, -2]}>
          <boxGeometry args={[0.2, 3, 14]} />
          <meshStandardMaterial color="#0f172a" transparent opacity={0.3} roughness={0.5} />
        </mesh>
      ...
      <group ref={floor3Group}>
        {/* Floor 3 Slab - Semi-transparent architectural glass */}
        <mesh position={[0, 15.8, 0]} receiveShadow>
          <boxGeometry args={[42, 0.3, 30]} />
          <meshStandardMaterial
            color="#090d16"
            transparent
            opacity={isAll ? 0.35 : activeFloor === 3 ? 0.85 : 0.2}
            roughness={0.2}
            metalness={0.4}
          />
        </mesh>
        <gridHelper args={[40, 20, "#ec4899", "#1e293b"]} position={[0, 16.02, 0]} />

        {/* Pediatrics Playful Wall Accents */}
        <mesh position={[-6, 17.5, -2]}>
          <boxGeometry args={[0.2, 3, 12]} />
          <meshStandardMaterial color="#ec4899" transparent opacity={0.25} roughness={0.4} />
        </mesh>

        {/* Neurology Chamber */}
        <mesh position={[6, 17.5, -2]}>
          <boxGeometry args={[0.2, 3, 12]} />
          <meshStandardMaterial color="#8b5cf6" transparent opacity={0.25} roughness={0.3} />
        </mesh>
      ...
      {/* ========================================================================= */}
      {/* ROOF & HELIPAD (Base Y: 24) — Open Cutaway Design                          */}
      {/* ========================================================================= */}
      <group ref={roofGroup}>
        {/* Helipad Platform (Solid dedicated pad on West corner) */}
        <mesh position={[-10, 23.9, 0]} receiveShadow>
          <boxGeometry args={[14, 0.3, 14]} />
          <meshStandardMaterial color="#1e293b" roughness={0.6} />
        </mesh>

        {/* Helipad Landing Markings */}
        <mesh position={[-10, 24.08, 0]} rotation={[-Math.PI / 2, 0, 0]}>
          <circleGeometry args={[5, 32]} />
          <meshStandardMaterial color="#ef4444" roughness={0.4} />
        </mesh>
        <mesh position={[-10, 24.1, 0]} rotation={[-Math.PI / 2, 0, 0]}>
          <ringGeometry args={[4.4, 4.8, 32]} />
          <meshBasicMaterial color="#ffffff" side={THREE.DoubleSide} />
        </mesh>

        {/* Perimeter Roof Crown / Trim (Keeps building silhouette without blocking beds) */}
        <mesh position={[0, 24.2, 14.8]}>
          <boxGeometry args={[42, 0.5, 0.2]} />
